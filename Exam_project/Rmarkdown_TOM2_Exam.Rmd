...
Notice! This Rmarkdown/html file contains the project on Danish vacation habits 1867 - 1881 and all technical explanation hereto. For historical explanation on this study please read the file 'Danish_vacation_habits_19th'. Find it here: (INDSÆT WEBLINK)

Notice! This study will try to follow the FAIR-principles so it can be used by others.
...
To prepare R (in the RStudio workspace) for historical sources that later on will be downloaded hereto,it is important to either install or download in library the packages needed. For instance the package "tidyverse" should be used. If it not or any of the other packages have been used before then install it like this: 

Install.packages(tidyverse)

If it just needs to be downloaded from the library please replace “install.packages” above with “library”. This is also a step required after installation.  
...
Now it is quite clear to see how the frequency evolves through time. What this means in a historical context will in the Report (HER HAR DU DEN IGEN) be clarified. 
...
The result of the textmining can be read in the report (INDSÆT NAVN HER) !!!!!!!
...
After the code has been runed the csv-file can know be found in the 'data' folder. As mentioned above the next step of this process will take place in OpenRefine. I use the 3.6.2 version of OpenRefine. See the PROJECT DOCUMENT HERE FOR EXPLANATION OF THE PROCESS. 
