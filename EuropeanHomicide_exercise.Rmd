...
The plot with Danish Monarchs shows that there is an increasing trend in the duration period 
whereas the trend with the homicides heavily declining.  

Visualizing
```{r eval = TRUE}
Danish_monarchs <- read.csv2("data/W45_spreadsheet_final.csv")

  Danish_monarchs <- Danish_monarchs %>% 
  mutate(
    mid_year = end_reign - (end_reign - start_reign)/2)
  
Danish_monarchs %>% 
  ggplot(aes(x = mid_year, y = reign_duration))+
  geom_smooth()
```
...
In booth plots I have changed Entity to Country by expand the labs command with the command color="Country).

2) Rmarkdown: 

  - edit the author of the document, and convert 'Final Tasks' into heading #2 (like the other headings) --> look over the Rmarkdown. 
...
Well it is hard to definie because it depends on what you understand by being civilized. Take a look on the visulasation of homicides, that as earlier stated, is a declining factor for all the countries included. So if we define civilized as not killing people, which would be a fair argument to take, yes we on one point can argue that we have become more civilized. When looking at the Danish Monarch visualization the duration period is expanding which can have connection to the fact that there are less homicides ergo fewer monarchs in the recent time has been killed.   